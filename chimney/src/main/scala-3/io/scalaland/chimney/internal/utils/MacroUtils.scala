...
object MacroUtils:
  ...
  def showTypeExpr[T: Type](using quotes: Quotes): Expr[String] = showTypeImpl[T]
  ...
  private def reportErrorAtPathWithTypeImpl[P <: String: Type, Error <: String: Type, T: Type](constantPart: Expr[String])(using q: Quotes): Expr[Nothing] =
    ...
    (Type.valueOfConstant[P], constantPart.value, Type.valueOfConstant[Error]) match
      case (Some(path), Some(constantPart), Some(error)) =>
        report.errorAndAbort(s"$constantPart at $path, type in question: ${Type.show[T]}, error: ${error}")
      case (p, cp, e) =>
        report.error(s"WTF: got $p, $cp, $e")
        report.errorAndAbort("Unable to produce nice error, bug in library")
