...
object CoproductDerive:
  ...
  inline def derived[From, To, Config <: Tuple, Path <: String](inline config: TypeDeriveConfig[Config, _, Path])(using fm: Mirror.SumOf[From], tm: Mirror.SumOf[To]): Transformer[From, To] =
    DeriveUtils.transformerWith[From,  To] { from => findACase[From, To, fm.MirroredElemTypes, tm.MirroredElemTypes, fm.MirroredElemLabels, Config, 0](config)(from) }
  end derived

  inline def derivedF[F[+_], From, To, Config <: Tuple, Path <: String](inline config: TypeDeriveConfig[Config, _, Path])(using fm: Mirror.SumOf[From], tm: Mirror.SumOf[To], sup: TransformerFSupport[F]): TransformerF[F, From, To] =
    DeriveUtils.transformerWithF[F, From, To](from => findACaseF[F, From, To, fm.MirroredElemTypes, tm.MirroredElemTypes, fm.MirroredElemLabels, Config, 0](config)(from))
  end derivedF

  inline def findACase[From, To, FromLeft <: Tuple, ToLeft <: Tuple, Labels <: Tuple, Config <: Tuple, Position <: Int](inline config: TypeDeriveConfig[Config, _, _])(from: From)(using fm: Mirror.SumOf[From]): To =
    inline (erasedValue[FromLeft], erasedValue[ToLeft], erasedValue[Labels]) match
      ...
      case _: (from *: _, EmptyTuple, _) =>
        inline extractInstanceAt[Config, from](config.asInstanceOf[TypeDeriveConfig[Config, _, _]]) match
          case f: Some[`from` => tt] => f.get.asInstanceOf[From => To](from)
          case _ =>
            inline config match
  ...
  inline def findACaseF[F[+_], From, To, FromLeft <: Tuple, ToLeft <: Tuple, Labels <: Tuple, Config <: Tuple, Position <: Int](
  ...
  private transparent inline def extractInstanceAtF[F[+_], Config <: Tuple, FF](inline config: TypeDeriveConfig[Config, _, _]) =
    inline erasedValue[InstanceConfigOf[Config, FF]] match
      case _: TransformerCfg.CoproductInstance[FF, tt] =>
        Some(config.instances((MacroUtils.showTypeVal[FF], MacroUtils.showTypeVal[tt])).asInstanceOf[FF => tt])
      case _: TransformerCfg.CoproductInstanceF[FF, tt] =>
        Some(config.instances((MacroUtils.showTypeVal[FF], MacroUtils.showTypeVal[tt])).asInstanceOf[FF => F[tt]])
      case _ =>
        None
  end extractInstanceAtF

  private transparent inline def extractInstanceAt[Config <: Tuple, FF](inline config: TypeDeriveConfig[Config, _, _]) =
    inline erasedValue[InstanceConfigOf[Config, FF]] match
      case _: TransformerCfg.CoproductInstance[FF, tt] =>
        Some(config.instances((MacroUtils.showTypeVal[FF], MacroUtils.showTypeVal[tt])).asInstanceOf[FF => tt])
      case _ =>
