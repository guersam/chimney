...
private[dsl] trait ScalaVersionSpecificTransformerFDefinition[F[+_], From, To, C <: TransformerCfg, Flags <: TransformerFlags] {
  ...
  def withFieldConst[T, U](
      selector: To => T,
      value: U
  ): TransformerFDefinition[F, From, To, _, Flags] =
  ...
  /** Use `f` to calculate the (missing) wrapped coproduct instance when mapping one coproduct into another
    ...
    *   [[io.scalaland.chimney.dsl.TransformerFDefinition]]
    */
  def withCoproductInstanceF[Inst](f: Inst => F[To]): TransformerFDefinition[F, From, To, _, Flags] =
